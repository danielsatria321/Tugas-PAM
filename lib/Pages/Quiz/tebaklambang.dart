...
class EmblemQuizPage extends StatefulWidget {
  final Map<String, dynamic> userData;

  const EmblemQuizPage({super.key, required this.userData});

  @override
  State<EmblemQuizPage> createState() => _EmblemQuizPageState();
}

class _EmblemQuizPageState extends State<EmblemQuizPage> {
  List countries = [];
  ...
  Future<void> fetchCountries() async {
    const url =
        'https://restcountries.com/v3.1/all?fields=name,coatOfArms,flags';
    try {
      final response = await http.get(Uri.parse(url));
      if (response.statusCode == 200) {
        List data = json.decode(response.body);

        // Hanya ambil negara yang memiliki lambang (coat of arms)
        data = data
            .where(
              (c) =>
                  c['coatOfArms'] != null &&
                  c['coatOfArms']['png'] != null &&
                  (c['coatOfArms']['png'] as String).isNotEmpty &&
                  c['name'] != null &&
                  c['name']['common'] != null,
            )
            .toList();
  ...
  void _generateOptions() {
    questionOptions = countries.map((country) {
      String correct = country['name']['common'];
      ...
      while (opts.length < 4) {
        ...
        if (!opts.contains(randomName)) opts.add(randomName);
      }
  ...
  Future<void> _saveQuizResults() async {
    ...
    try {
      ...
      if (userId != null) {
        ...
        print(
          '✅ Emblem Quiz completed! Score: $score/${countries.length}, XP earned: $xpEarned',
        );
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              const Text(
                "Lambang negara milik negara mana?",
