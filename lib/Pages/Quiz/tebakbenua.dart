...
class ContinentQuizPage extends StatefulWidget {
  final Map<String, dynamic> userData;

  const ContinentQuizPage({super.key, required this.userData});

  @override
  State<ContinentQuizPage> createState() => _ContinentQuizPageState();
}

class _ContinentQuizPageState extends State<ContinentQuizPage> {
  List countries = [];
  ...
  Future<void> fetchCountries() async {
    const url =
        'https://restcountries.com/v3.1/all?fields=name,continents,coatOfArms';
    try {
      final response = await http.get(Uri.parse(url));
      if (response.statusCode == 200) {
        List data = json.decode(response.body);

        // Ambil hanya negara yang punya coatOfArms
        data = data
            .where(
              (c) =>
                  c['coatOfArms'] != null &&
                  c['coatOfArms']['png'] != null &&
                  c['continents'] != null,
            )
            .toList();
  ...
  void _generateOptions() {
    optionsList = countries.map((country) {
      String correct = country['continents'][0];
      List<String> opts = [correct];

      // Tambah opsi salah acak sampai total 4
      while (opts.length < 4) {
        String random = (allContinents.toList()..shuffle()).first;
        if (!opts.contains(random)) opts.add(random);
      }
  ...
  Future<void> _saveQuizResults() async {
    ...
    try {
      ...
      if (userId != null) {
        ...
        print(
          '✅ Continent Quiz completed! Score: $score/${countries.length}, XP earned: $xpEarned',
        );
  ...
  Widget build(BuildContext context) {
    ...
    final coatUrl = question['coatOfArms']['png'];
    final correctAnswer = question['continents'][0];
    final options = optionsList[currentQuestion];
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              const Text(
                "Negara ini berada di benua apa?",
