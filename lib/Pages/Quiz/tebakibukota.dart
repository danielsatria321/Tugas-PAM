...
class CapitalQuizPage extends StatefulWidget {
  final Map<String, dynamic> userData;

  const CapitalQuizPage({super.key, required this.userData});

  @override
  State<CapitalQuizPage> createState() => _CapitalQuizPageState();
}

class _CapitalQuizPageState extends State<CapitalQuizPage> {
  List countries = [];
  ...
  Future<void> fetchCountries() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        // Hanya ambil negara yang punya ibu kota
        data = data
            .where(
              (c) =>
                  c['capital'] != null &&
  ...
  void _generateOptions() {
    questionOptions = countries.map((country) {
      String correct = country['capital'][0];
      List<String> opts = [correct];

      while (opts.length < 4) {
        String randomCapital =
            countries[(countries.indexOf(country) + opts.length) %
                    countries.length]['capital'][0]
                .toString();
        if (!opts.contains(randomCapital)) opts.add(randomCapital);
      }
  ...
  Future<void> _saveQuizResults() async {
    ...
    try {
      ...
      if (userId != null) {
        ...
        print(
          '✅ Capital Quiz completed! Score: $score/${countries.length}, XP earned: $xpEarned',
        );
  ...
  Widget build(BuildContext context) {
    ...
    final correctAnswer = question['capital'][0];
