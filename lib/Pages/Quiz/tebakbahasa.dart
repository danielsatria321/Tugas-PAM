...
class LanguageQuizPage extends StatefulWidget {
  ...
  const LanguageQuizPage({super.key, required this.userData});
...
class _LanguageQuizPageState extends State<LanguageQuizPage> {
  ...
  Future<void> fetchCountries() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        // Ambil hanya negara dengan data bahasa dan bendera
        data = data
            .where(
              (c) =>
                  c['languages'] != null &&
                  (c['flags']?['png'] != null || c['flags']?['svg'] != null),
            )
            .toList();
  ...
  void _generateOptions() {
    ...
    for (var c in countries) {
      allLanguages.addAll((c['languages'] as Map).values.cast<String>());
    }

    allLanguages = allLanguages.toSet().toList();

    optionsList = countries.map((country) {
      String correct = (country['languages'] as Map).values.first;
      List<String> opts = [correct];

      while (opts.length < 4) {
        String random = (allLanguages.toList()..shuffle()).first;
  ...
  Future<void> _saveQuizResults() async {
    ...
    try {
      ...
      if (userId != null) {
        ...
        print(
          ' Language Quiz completed! Score: $score/${countries.length}, XP earned: $xpEarned',
        );
  ...
  Widget build(BuildContext context) {
    ...
    final flagUrl = question['flags']['png'] ?? question['flags']['svg'];
    final correctAnswer = (question['languages'] as Map).values.first;
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              const Text(
                "Apa bahasa resmi utama negara ini?",
